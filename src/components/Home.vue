<template>
  <div class="card-container">
    ...
    <div class="card card--special card--dark inline-flex flex-col">
      ...
      <a class="rounded-full shadow-sm bg-emerald-300 text-slate-600 p-2 inline-block mt-3 mx-auto" href="https://docs.google.com/forms/d/e/1FAIpQLSe9KC-zzcd89iinRnFIZoK7rjcRsFJjsFwFz3u6Wcad3rS2WQ/viewform">Dodaj meeting</a>
    </div>
    <div class="backdrop" v-if="popUpList.some(e=>e == true)"></div>
    <div v-for="(card,index) in cards" :key="card['Sygnatura czasowa']" class="card card--pop-up" :class="{'card--pop-up-active':popUpList[index]}">

      <span>{{card['Dzień tygodnia']}} {{card['Godzina rozpoczęcia']}}</span>
      <h2 v-if="card['Nazwa Meetingu'] != '' " class="text-lg font-bold">{{ card['Nazwa Meetingu'] }}</h2>
...
import { sync, Storage, getRows, getHeaders, getPreparedCards, getPreparedCardsWithLonLat, getSortedMiniCards } from '../composables/fetchMeetings.js'
...
console.clear();
    ...
    const togglePopUp = (index) => {
      popUpList.value[index]= !popUpList.value[index]
    }
